...
OPTIONAL PARAMS:    cmap     = color map used to plot the arrows
                    ...
                    svfolder/svfile = the folder and file name of the saved video
                    qv_scale = length of the arrows, smaller value = larger arrows
'''
import numpy         as np
...
-------------------------------------------------------------------------------
'''
def animate_quiver(xx, yy, data_U, data_V, time, cmap=cm.OrRd, nstep=100, fid=None, \
         ptitle=None, svfolder= './videos/', svfile='temp_quiver.mp4', qv_scale=3, unit='(unit)'):
    ...
    if ptitle != None:
        ...
        fig.suptitle('file {0}, {1}'.format(fid,fn.fname_tds(fid, old=0,full=0)) \
            , y=yalign, fontsize=15)
    ...
    def generate_frame(i):
        ...
        frame_mag = [np.sqrt(u**2+v**2) for u,v, in zip(data_U[tt,:,:],data_V[tt,:,:])]
        local_max = np.amax(np.absolute(frame_mag))
        sc        = global_max/local_max
        frame     = ax.quiver(xx, yy, data_U[tt,:,:]*sc, data_V[tt,:,:]*sc, frame_mag, \
           scale=global_max/qv_scale, scale_units='xy', cmap=cmap)

        ### 1. Data has some value X.
        ### 2. Define sc, the scaling factor which all data need to be scaled by so that
        ###    arrows are proportion to global maximum. (data = X*sc)
        ### 3. Define scale=global_max, the scaling factor of the data. Data is divided
        ###    by this number so that the unit length is the scale_unit.
        ...
        print('\r Generating frame {0}/{1} ({2:.2f}%)...'.format(i+1,nframes,(i+1)/nframes*100), end='')
