...
def minmax_CH_plot(dim=5, npoints=100):
    ...
    uniform_entropy = -np.sum([ii*np.log(ii) for ii in uniform_prob]) \
        / np.log(2)
    ...
    # MIN COMPLEXITY
    # probability looks like: 1 element with probability pmax, others have
    # probability (1-pmax)/(Nf-1) and pmax can vary from 1./N to 1 -> each one
    # of these will correspond to a different entropy H. And for each of these
    # H's one can calculate the complexity. (See also Maggs' paper.)
    pmax0 = [ii/df * (1 - 1/df) + 1/df for ii in np.arange(df)]

    # alog(2) factors cancel out
    Hmin_arr = [-1/np.log(df) * (jj*np.log(jj) + (1-jj)*np.log((1-jj)/(df-1)))
                for jj in pmax0]

    # Now calculate Cmin, need sum_probabity -> one element will have
    #  (pmax+1/Nf)/2 others will have ((1-pmax)/(Nf-1)+1./Nf)/2
    ...
    sum_entropy0 = [-(aa*np.log(aa) + (df-1)*bb*np.log(bb)) / np.log(2)
                    for aa, bb in zip(pmax_sum0, pmin_sum0)]
    shanDmin = [aa - bb/2 - uniform_entropy/2 for aa, bb in zip(sum_entropy0,
                entropy_min)]
    ...
    for mm in np.arange(1, df-1):
        pmax1 = [ii/npmax * (1-1/(df-mm)) + 1/(df-mm) for ii in
                 np.arange(npmax)]
        Hmax[mm, :] = [-1/np.log(df) * (jj * np.log(jj) + (1-jj) *
                       np.log((1-jj)/(df-mm-1))) for jj in pmax1]
        entropy_max = [jj * np.log(df)/np.log(2) for jj in Hmax[mm, :]]

        # Now calculate Cmax, sum probability, one element with probability
        #  (pmax+1./Nf)/2, m elements with probability 1./2/Nf and Nf-m-1
        #  elements with probability ( (1-pmax)/(Nf-m-1) + 1./Nf ) / 2.
        ...
        shanDmax = [aa - bb/2 - uniform_entropy/2 for aa, bb in
                    zip(sum_entropy1, entropy_max)]
...
def plot_CH_plane(dim=5, blank=0):
    ...
    fig = tbx.prefig(figsize=(16, 9), xlabel='BP entropy $H$',
...
def lorenz_attractor(sigma=10, beta=8/3, rho=28, dim=5):  # show=0
    ...
    #       ax.plot(x[i:i+s+1], y[i:i+s+1], z[i:i+s+1], color=(1,c[i],0),
    #               alpha=0.4)
...
def double_pendulum_deriv(X, t, m=1, l=1, g=10):
    ...
    thxd1 = 6.0/(m*l**2) * (2*thdx0 - 3*np.cos(thx0-thy0)*thdy0) \
        / (16-9*(np.cos(thx0-thy0))**2)
    thyd1 = 6.0/(m*l**2) * (8*thdy0 - 3*np.cos(thx0-thy0)*thdx0) \
        / (16-9*(np.cos(thx0-thy0))**2)
...
def double_pendulum(m=1, l=1, g=10, dim=5):
    ...
    t = np.linspace(0, tmax, n)
    f = odeint(double_pendulum_deriv, (thx0, thy0, thdx0, thdy0), t,
               args=(m, l, g))
...
def fBm_gen(dim=5):
    ...
    # angular frequency 0 to pi
    freq = [((ii/2)+1)*2*np.pi/siglen for ii in range(siglen)]
    data = np.empty([nhe, 2])

    for ii in range(nhe):
        tbx.progress_bar([ii+1], [nhe], label=['Hurst exponents'],
                         header='Generating fBm ')
        h2 = 2*he[ii]
        ...
        v[1:mid] = [np.sqrt(0.5*sdf[jj]) * np.complex(wr[2*jj-1], wr[2*jj]) for
                    jj in range(1, mid)]
        v[mid] = np.sqrt(sdf[mid]) * wr[siglen-1]
        v[mid+1:siglen] = [np.conj(v[siglen-jj]) for jj in range(mid+1,
                           siglen)]
