...
AUTHOR:      	swjtang
DATE:        	06 Feb 2019
DESCRIPTION: 	Coordinate transform of the XY probe drive on the 180E ICP machine.
...
def op(xarr, yarr, length1, length2, debug=0):
	nx = len(xarr)
	ny = len(yarr)
	y = [[(length1+xarr[ii])/length2*yarr[jj] for jj in range(ny)] for ii in range(nx)]
	x = [[-((length1+xarr[ii])*np.sqrt(1-(yarr[jj]/length2)**2)-length1) for jj in range(ny)] for ii in range(nx)]

	theta = [[np.arcsin(yarr[jj]/length2) for jj in range(ny)] for ii in range(nx)] 
