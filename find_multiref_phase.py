        ...
        for yy in range(ny):
            for ichan in range(nchan):
                ...
                for ss in range(nshots):
                    tbx.progress_bar([xx+1, yy+1, ss+1, ichan+1], [nx, ny,
                                     nshots, nchan], label=['nx', 'ny',
                                     'nshots', 'nchan'], header='Conditionally'
                                     ' averaging data...')
...
# -----------------------------------------------------------------------------
def find_multiref_phase(data, time, ref=[0, 0, 0], trange=None, arg2pi=None,
                        db=0, dbshot=[4, 5, 0, 1]):
    # Extract parameters from inputs ---------------------------
    # data dimensions
    nt, nx, ny, nshots, nchan = data.shape
    # reference shot ID (x, y, shot)
    ...
    # start ----------------------------------------------------
    # the array to store the correlated data
    lagarr = np.empty([nx, ny, nshots, nchan])
    # the array to determine if a shot should be skipped
    passarr = np.zeros([nx, ny, nshots, nchan])
            ...
            for yy in range(ny):
                for ss in range(nshots):
                    for ichan in range(nchan):
                        ...
                        laginfo = determine_lag_time(refsig, sig1,
                                                     arg2pi=arg2pi)
                        if laginfo['error'] == 0:
