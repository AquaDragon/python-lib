...
AUTHOR:      	swjtang
DATE:        	28 Aug 2020
DESCRIPTION: 	functionally similar to read_lapd_data_newsis_ver4
INPUTS:			
'''
# from bapsflib import lapd
import h5py, re
import numpy as np
from .toolbox import show_progress_bar, qprint
...
def read_lapd_data(fname, daqconfig=0, rchan=None, rshot=None,\
	...
	if device_check[1]:
		...
		def print_channels(sisboard_info, sisid):
			...
			if nchan > 0:
				qprint(quiet, '---------- SIS {0} list of enabled channels ({1}) ----------'\
					.format(sisid, nchan))
				for ii in range(nchan):
					te = [ii, boardlist[ii], chanlist[ii], dtypelist[ii]]
					qprint(quiet, '[{0:2}] Board {1}, Channel {2}: {3}'.format(te[0], te[1], te[2], te[3]))
		...
		if sisid == 3302:
			...
			qprint(quiet, 'Reading SIS 3302...')
		elif sisid == 3305:
			sisboard_info = sis3305_temp
			boardlist, chanlist, dtypelist, nchan = bl3305, cl3305, dtl3305, nch3305
			qprint(quiet, 'Reading SIS 3305...')
		else:
			qprint(quiet, 'None of the SIS 3302/3305 boards have any enabled channels!')
			return None

		# determine sampling rate of the board (use first board) --------------
		dt = sisboard_info['dt']
		nt = sisboard_info['nt']
		if len(np.unique(dt)) > 1 : qprint(quiet, 'Different boards have different sampling rates!')
		if len(np.unique(nt)) > 1 : qprint(quiet, 'Different boards have different number of samples!')
		if len(np.unique(dt)) > 1 or len(np.unique(nt)) > 1 : 
			qprint(quiet, 'Choosing sampling time of channel with index '+str(tchannum)+'...')
	...
	qprint(quiet, '-------------------------------------------------------------')
	qprint(quiet, 'Data geometry = '+geom)
	qprint(quiet, 'Read Channels = '+'   '.join([str(ii) for ii in rchan]))
	qprint(quiet, 'Shot range    = '+str(rshot[0])+' to '+str(rshot[1]))
	qprint(quiet, 'X value range = '+str(xrange[0])+' to '+str(xrange[1]))

	if geom == 'xz-plane':
		qprint(quiet, 'Z value range = '+str(zrange[0])+' to '+str(zrange[1]))
	elif geom == 'xyz-volume':
		qprint(quiet, 'Y value range = '+str(yrange[0])+' to '+str(yrange[1]))
		qprint(quiet, 'Z value range = '+str(zrange[0])+' to '+str(zrange[1]))
	else:
		qprint(quiet, 'Y value range = '+str(yrange[0])+' to '+str(yrange[1]))
	...
	# qprint(quiet, data_subgroup_names, data_dataset_names)
			...
			for iiz in range(zrange[0], zrange[1]+1):
				for iishot in range(rshot[0], rshot[1]+1):
					...
					for jj in range(len(rchan)):
						show_progress_bar([iix-xrange[0]+1, iiy-yrange[0]+1, iiz-zrange[0]+1, \
	...
	qprint(quiet, '!!! '+hlabel+' = '+str(dataset.shape))
