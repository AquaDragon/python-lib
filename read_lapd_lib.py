...
def lapd_motionlist_config(fname, motionid=0, module='6K Compumotor', quiet=0):
	...
	qprint(quiet, 'Reading data from motion list {0}'.format(motion_dataset_names[motionid]))
	...
	if len(motion_list_shot) % nxnynz != 0:    # error checking for incomplete datasets
		qprint(quiet, '!!! This is a dataset from an incomplete datarun.')
		geom = 'incomplete'
...
def get_sisboard_config(fname, sisconfig_name, sid=0, quiet=0):
	...
	if nboards > 0 :
		qprint(quiet, 'SIS {0} boards used = {1}'.format(sid, sis_board_numbers))
	else :
		qprint(quiet, 'No '+str(sid)+' boards used.')
		return None
	...
	for ii in range(nboards):
		...
		# qprint(quiet, iboard, iconfig, board_config_name)
		# qprint(quiet, list(ff[board_config_group].attrs))
		...
		qprint(quiet, 'SIS '+str(sid)+' effective clock rate: Board '+str(iboard+1) + \
				' = '+ str(1/(1e6*dt)) + ' MHz')
