...
-------------------------------------------------------------------------------
DESCRIPTION:    Given the radius and center of circle, find coordinates of L 
                points on a circle.
...
''' ---------------------------------------------------------------------------
DESCRIPTION:    Define the radial eigenfunction to be used in the calculation.
INPUTS:         r = radius from center of cylinder
'''
...
''' ---------------------------------------------------------------------------
DESCRIPTION:    Plot the potential created by summing the radial eigenfunction.
INPUTS:         center  = 2D coordinate for the center of the circle.
                Lmode   = Azimuthal mode number
                phase   = A 1D array of phases used in potential calculation.
OPTIONAL INPUT: x, y    = 1D arrays that contain position information. If not
                          set, default values are used.
                eigen   = A 2D array that contains the map of the radial
                          eigenfunction. Uses the eigenfunction in this program
                          if not specified.
                display = Set to 1 to show the plot of radial eigenfunction.

'''
def get_potential_eigen(x=[], y=[], eigen=[], center=[0,0], Lmode=1, display=0, phase=[0]):
    if len(x) == 0: x = np.linspace(-10, 10, 100)
    if len(y) == 0: y = np.linspace(-10, 10, 200)
    ...
    if len(eigen) == 0: 
        ...
        eigen_arr = eigen
    if display == 1: plot_radial_eigen(x, y, eigen_arr)

    if len(phase) == 1:
        pot_L = eigen_arr * np.exp(1j*(Lmode*th_arr - phase))
    else:
        pot_L = np.array([eigen_arr * np.exp(1j*(Lmode*th_arr - ph)) for ph in phase])
...
''' ---------------------------------------------------------------------------
DESCRIPTION:    Functions to help visualize by making contourf plots.
INPUTS:         
'''
